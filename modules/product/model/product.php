...
class Product{
	...
	public function __construct(){
        ...
        $this->menu                   = new system\Model('vi_menu');
	}
   ...
   public function getProductLienQuan($id,$category){
    ...
    public function getToursm($cate_id){
        $sql = "SELECT * FROM vi_product_basic WHERE category LIKE '%".$cate_id."%' AND status = 1 AND hot = 1  ORDER BY order_by DESC LIMIT 0,6";
        $result = $this->product->rawQuery($sql);
        return $result;
    }
    public function getKhampha($cate_id){
        $sql = "SELECT * FROM vi_posts WHERE cate_id LIKE '%".$cate_id."%' AND status = 1 AND hot = 1  ORDER BY order_by DESC LIMIT 0,6";
        $result = $this->posts->rawQuery($sql);
        return $result;
    }
    public function getMenuNuoc($position){
        $this->menu->where("status",1);
