...
class ProductController extends Controller{
	...
	public function index(){
        if($_GET['id']){
            ...
            if (!empty($all_pages )) {
             ...
             /**
             * Xu ly menu theo cate
             */
            $backcrumb    = $this->modelProduct->getBreadcrumbsCategory($id);
			$cateEnd	  = array_pop($backcrumb);
			$this->view->data['cateTitle'] = $cateEnd;
			$check_cate        			   = $cateEnd['id'];
			if ($check_cate ==1) {
				$this->view->data['listNamecate']  = $this->modelProduct->getMenuNuoc(10);
			}elseif($check_cate ==2){
			 	$this->view->data['listNamecate']  = $this->modelProduct->getMenuNuoc(11);
			}
	...
	// die("ok");
	public function tourisma(){
		global $_web;
		$this->view->data['data']     = $this->modelProduct->getToursm(',1,');
        $this->view->data['khampha']  = $this->modelProduct->getKhampha(',2,');

		$this->view->title       = "Tour trong nước";
		$this->view->render('tourism');
	}
	public function abroadb(){
		global $_web;
		$this->view->data['data'] = $this->modelProduct->getToursm(',2,');
        $this->view->data['khampha']  = $this->modelProduct->getKhampha(',3,');

		$this->view->title       = "Tour ngoài nước";
		$this->view->render('abroadb');
	}
